...
#if ((_MSC_VER >= 1700 || __cplusplus >= 201103L) && !defined(IW_SDK))
#	include <thread>
#	include <mutex>
...
namespace { // private module-only namespace
	...
	public:
		...
		void poll(int timeout, WSErrorCallback errorCallback, void* userData)  // timeout in milliseconds
        {
            ...
            if(readyState == CONNECTING)
            {
                ...
                else if( ipLookup == keFailed )
                {
					...
					#else
						errorCallback(WSError(WSError::DNS_LOOKUP_FAILED, "DNS Lookup, TLS-handshake or websocket-handshake failed"), userData);
					#endif
                }
            }
        ...
        static void* _s_dns_Lookup(void *ptr)
        {
			...
			#else
			// Note: we're passing nullptr to doConnect2() - thos callbacks are called in poll()
			if (!self->socket->connect(self->m_host.c_str(), static_cast<short>(self->m_port)) || !self->doConnect2(nullptr, nullptr))
			{
				self->ipLookup = keFailed;
			}
			else
